...
const { Client, GatewayIntentBits, Partials, AttachmentBuilder, ChannelType } = require("discord.js");
...
function defaultUser(passwordHash) {
  return {
    ...
    botPanel: {
      ...
      tagTrigger: "tag",
      adTrigger: "reklam",
      dmMessage: "Send me your Discord server invite link.",
      video: null,
      adRequests: {},
      adCooldowns: {},
      running: false,
...
function safeUser(username, user) {
  return {
    ...
    botPanel: {
      tokenSaved: !!user.botPanel?.botToken,
      adChannelId: user.botPanel?.adChannelId || "",
      tagTrigger: user.botPanel?.tagTrigger || "tag",
      adTrigger: user.botPanel?.adTrigger || "reklam",
      dmMessage: user.botPanel?.dmMessage || "Send me your Discord server invite link.",
      videoName: user.botPanel?.video?.originalname || "",
      running: !!user.botPanel?.running,
      lastError: user.botPanel?.lastError || ""
    }
  };
}
...
function getInviteLink(text) {
  const value = String(text || "").trim();
  const match = value.match(/https?:\/\/(?:www\.)?discord\.com\/invite\/[A-Za-z0-9-]+|https?:\/\/discord\.gg\/[A-Za-z0-9-]+|(?:^|\s)discord\.gg\/[A-Za-z0-9-]+|(?:^|\s)discord\.com\/invite\/[A-Za-z0-9-]+/i);
  if (!match) return "";
  return match[0].trim();
}
...
const AD_COOLDOWN_MS = 5 * 60 * 60 * 1000;

function formatWaitTime(ms) {
  const totalMinutes = Math.max(1, Math.ceil(ms / 60000));
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
  if (hours && minutes) return `${hours} hour(s) and ${minutes} minute(s)`;
  if (hours) return `${hours} hour(s)`;
  return `${minutes} minute(s)`;
}

function ensureBotPanelState(user) {
  if (!user.botPanel) user.botPanel = {};
  if (!user.botPanel.adRequests || typeof user.botPanel.adRequests !== "object") user.botPanel.adRequests = {};
  if (!user.botPanel.adCooldowns || typeof user.botPanel.adCooldowns !== "object") user.botPanel.adCooldowns = {};
  return user.botPanel;
}

function resolveBotVideoPath(video) {
  if (!video) return "";
  const candidates = [];
  if (video.filename) candidates.push(path.join(UPLOAD_DIR, video.filename));
  if (video.path) {
    candidates.push(video.path);
    candidates.push(path.join(__dirname, video.path));
    candidates.push(path.join(UPLOAD_DIR, path.basename(video.path)));
  }
...
function stopBot(username) {
  const client = botClients.get(username);
  if (client) {
    client.destroy();
...
async function listBotChannels(username) {
  const client = botClients.get(username);
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    fetched.forEach((channel) => {
      if (!channel) return;
      const textType = channel.type === ChannelType.GuildText || channel.type === ChannelType.GuildAnnouncement;
      if (!textType) return;
...
async function startBot(username) {
  ...
  const db = loadDb();
  const user = db.users[username];
  if (!user?.botPanel?.botToken) return { success: false, message: "Bot token is empty." };

  user.botPanel.lastError = "";
  user.botPanel.running = false;
  db.users[username] = user;
  saveDb(db);
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      const cfg = freshUser?.botPanel || {};
      const content = String(message.content || "").trim();
      const lower = content.toLowerCase();
      const tagTrigger = normalizeTrigger(cfg.tagTrigger, "tag");
      const adTrigger = normalizeTrigger(cfg.adTrigger, "reklam");

      if (message.channel?.type === 1) {
        const freshDbForDm = loadDb();
        const freshUserForDm = freshDbForDm.users[username];
        const dmCfg = ensureBotPanelState(freshUserForDm);
        const userId = message.author.id;
        const invite = getInviteLink(content);
        const pending = !!dmCfg.adRequests[userId];
        const lastAdAt = Number(dmCfg.adCooldowns[userId] || 0);
        const waitMs = AD_COOLDOWN_MS - (Date.now() - lastAdAt);

        if (lastAdAt && waitMs > 0) {
          await message.reply(`Please wait ${formatWaitTime(waitMs)} before requesting another ad. Go to our Discord server and type \`reklam\` again later, then come back and send your Discord server invite link.`);
          return;
        }

        if (!pending) {
          await message.reply("Please go to our Discord server and type `reklam` first. Then come back here and send me your Discord server invite link.");
          return;
        }

        if (!invite) {
          await message.reply("Only Discord invite links are accepted: discord.gg/... or discord.com/invite/...");
          return;
        }
        if (!dmCfg.adChannelId) {
          await message.reply("The ad channel is not configured yet in the web panel.");
          return;
        }
        const target = await client.channels.fetch(String(dmCfg.adChannelId).trim()).catch(() => null);
        if (!target || !target.isTextBased()) {
          await message.reply("The selected ad channel could not be found. Please check the channel in the web panel.");
          return;
        }
        await target.send(invite);
        dmCfg.adCooldowns[userId] = Date.now();
        delete dmCfg.adRequests[userId];
        freshDbForDm.users[username] = freshUserForDm;
        saveDb(freshDbForDm);
        await message.reply("Your Discord server has been advertised successfully. If you want another advertisement, please wait 5 hours, go back to our Discord server, type `reklam` again, and send me your Discord server invite link again.");
        return;
      }

      if (lower === adTrigger) {
        const freshDbForAd = loadDb();
        const freshUserForAd = freshDbForAd.users[username];
        const adCfg = ensureBotPanelState(freshUserForAd);
        adCfg.adRequests[message.author.id] = Date.now();
        freshDbForAd.users[username] = freshUserForAd;
        saveDb(freshDbForAd);
        await message.reply(adCfg.dmMessage || "Please send me your Discord server invite link in DM.");
        return;
      }

      if (lower === tagTrigger) {
        const video = cfg.video;
        const videoPath = resolveBotVideoPath(video);
        if (!videoPath) {
          await message.reply("The video has not been uploaded from the web panel yet.");
          return;
        }
        const attachment = new AttachmentBuilder(videoPath, { name: video.originalname || "video.mp4" });
        await message.reply({ files: [attachment] });
      }
    } catch (error) {
      console.error("Bot message error:", error.message || error);
    }
  });

  client.once("ready", () => {
    const readyDb = loadDb();
    if (readyDb.users[username]) {
      readyDb.users[username].botPanel.running = true;
      readyDb.users[username].botPanel.lastError = "";
      saveDb(readyDb);
    }
    console.log(`Discord bot is ready for ${username}: ${client.user.tag}`);
  });

  client.on("error", (error) => {
    const errDb = loadDb();
    if (errDb.users[username]) {
      errDb.users[username].botPanel.lastError = error.message || "Discord bot error.";
      errDb.users[username].botPanel.running = false;
      saveDb(errDb);
    }
  });

  botClients.set(username, client);
  try {
    await client.login(user.botPanel.botToken);
    return { success: true, message: "Bot started." };
  } catch (error) {
    stopBot(username);
    const failDb = loadDb();
    if (failDb.users[username]) {
      failDb.users[username].botPanel.running = false;
      failDb.users[username].botPanel.lastError = error.message || "Bot token invalid.";
      saveDb(failDb);
    }
    return { success: false, message: "Bot could not start: " + (error.message || "Invalid token") };
  }
}
...
app.get("/api/bot-panel", requireAuth, (req, res) => {
  const db = loadDb();
  res.json({ success: true, user: safeUser(req.session.username, db.users[req.session.username]) });
});
...
app.get("/api/bot-channels", requireAuth, async (req, res) => {
  try {
    const channels = await listBotChannels(req.session.username);
    res.json({ success: true, channels });
  } catch (error) {
    res.status(400).json({ success: false, message: "Could not load channels: " + (error.message || "Bot is not ready."), channels: [] });
  }
});

app.post("/api/bot-panel", requireAuth, async (req, res) => {
  const db = loadDb();
  const user = db.users[req.session.username];
  if (!user.botPanel) user.botPanel = {};
  const oldToken = user.botPanel.botToken || "";
  const newToken = typeof req.body?.botToken === "string" && req.body.botToken.trim() ? req.body.botToken.trim() : oldToken;
  user.botPanel.botToken = newToken;
  user.botPanel.adChannelId = typeof req.body?.adChannelId === "string" ? req.body.adChannelId.trim() : (user.botPanel.adChannelId || "");
  user.botPanel.tagTrigger = typeof req.body?.tagTrigger === "string" && req.body.tagTrigger.trim() ? req.body.tagTrigger.trim() : "tag";
  user.botPanel.adTrigger = typeof req.body?.adTrigger === "string" && req.body.adTrigger.trim() ? req.body.adTrigger.trim() : "reklam";
  user.botPanel.dmMessage = typeof req.body?.dmMessage === "string" && req.body.dmMessage.trim() ? req.body.dmMessage : "Send me your Discord server invite link.";
  ensureBotPanelState(user);
  user.botPanel.lastError = "";
  db.users[req.session.username] = user;
  saveDb(db);

  if (user.botPanel.botToken) {
    const result = await startBot(req.session.username);
    const fresh = loadDb().users[req.session.username];
    return res.status(result.success ? 200 : 400).json({ success: result.success, message: result.message, user: safeUser(req.session.username, fresh) });
  }
  res.json({ success: true, message: "Bot settings saved. Add a bot token to start it.", user: safeUser(req.session.username, user) });
});

app.post("/api/bot-video", requireAuth, upload.single("botVideo"), async (req, res) => {
  if (!req.file) return res.status(400).json({ success: false, message: "Please choose a video first." });
  if (!String(req.file.mimetype || "").startsWith("video/")) {
    fs.unlink(req.file.path, () => {});
    return res.status(400).json({ success: false, message: "Only video files are accepted." });
  }
  const db = loadDb();
  const user = db.users[req.session.username];
  if (!user.botPanel) user.botPanel = {};
  const old = user.botPanel.video;
  const oldPath = resolveBotVideoPath(old);
  if (oldPath) fs.unlink(oldPath, () => {});
  user.botPanel.video = {
    filename: req.file.filename,
    path: req.file.path,
    originalname: req.file.originalname || "bot-video.mp4",
    mimetype: req.file.mimetype || "video/mp4",
    uploadedAt: new Date().toISOString()
  };
  db.users[req.session.username] = user;
  saveDb(db);
  res.json({ success: true, message: "Bot video uploaded and saved.", user: safeUser(req.session.username, user) });
});

app.post("/api/bot-stop", requireAuth, (req, res) => {
  stopBot(req.session.username);
  const db = loadDb();
  const user = db.users[req.session.username];
  if (!user.botPanel) user.botPanel = {};
  user.botPanel.running = false;
  db.users[req.session.username] = user;
  saveDb(db);
  res.json({ success: true, message: "Bot stopped.", user: safeUser(req.session.username, user) });
});
...
app.listen(PORT, () => {
  ...
  Object.keys(db.users).forEach((username) => {
    ...
    if (db.users[username].botPanel?.botToken) startBot(username);
  });
